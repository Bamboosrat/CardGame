...
public class AiPlayer : MonoBehaviour, IPlayerInterface
{
	...
	public void turn()
	{ //the ai's turn
		CardDisplay currDisc = GameManager.discard[GameManager.discard.Count - 1];
		...
		{
			GameObject.Find("GameManager").GetComponent<GameManager>().draw(1, this);
			drew = true;
		}
	...
	{ //recieves cards to the hand
		handList.Add(other);
	}
	public void turnEnd()
	{ //ends the turn
		GameManager cont = GameObject.Find("GameManager").GetComponent<GameManager>();
		if (drew)
		{
